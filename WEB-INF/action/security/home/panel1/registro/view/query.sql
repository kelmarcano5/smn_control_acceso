...
and
    r.app_id = p.app_id 
and
    (r.rolename = 'smn_registros' or r.rolename = 'smn_admin_coa' or r.rolename = 'smn_aprobador')
and	
	mi.service_id = s.service_id
and
	mi.menu_id = m.menu_id
and
	(upper(m.title) = upper('registro') or upper(m.title) = upper('registro (registrador)'))
and
