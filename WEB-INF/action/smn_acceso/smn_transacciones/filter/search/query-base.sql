select
	...
	CASE 
		when smn_base.smn_clase_auxiliar.smn_clase_auxiliar_id=1 then smn_base.smn_auxiliar.aux_num_doc_oficial||' - '|| smn_base.smn_auxiliar.aux_codigo||' '||smn_base.smn_auxiliar.aux_descripcion
		when smn_base.smn_clase_auxiliar.smn_clase_auxiliar_id=2 then smn_base.smn_auxiliar.aux_num_doc_oficial||' - '|| smn_base.smn_auxiliar.aux_codigo||' '||smn_base.smn_auxiliar.aux_descripcion
		when smn_base.smn_clase_auxiliar.smn_clase_auxiliar_id=3 then smn_base.smn_auxiliar.aux_num_doc_oficial||' - '|| smn_base.smn_auxiliar.aux_nombres||' '||smn_base.smn_auxiliar.aux_apellidos
	END AS personal,
	case
