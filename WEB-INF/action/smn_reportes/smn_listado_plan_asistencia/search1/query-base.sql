...
select DISTINCT rows.asi_personal_rf, rows.fecha, rows.tipo_movimiento, rows.hora_tope_evento, rows.hora_marcaje, rows.asi_fecha_registro, smn_control_acceso.smn_justificaciones.jus_descripcion as justificacion, smn_gip.smn_categoria_justificaciones.cju_nombre as smn_categoria_justificacion_rf from rows
left join smn_control_acceso.smn_justificaciones on smn_control_acceso.smn_justificaciones.smn_plan_asistencia_id = rows.smn_plan_asistencia_id
left join smn_control_acceso.smn_asistencia_comedor on smn_control_acceso.smn_asistencia_comedor.smn_asistencia_id = rows.smn_asistencia_id
inner join smn_gip.smn_categoria_justificaciones on smn_gip.smn_categoria_justificaciones.smn_categoria_justificaciones_id = smn_control_acceso.smn_justificaciones.smn_categoria_justificacion_id 
where smn_control_acceso.smn_justificaciones.jus_fecha_incidencia>=${fld:asi_fecha_registro_ini} and smn_control_acceso.smn_justificaciones.jus_fecha_incidencia<=${fld:asi_fecha_registro_fin}
