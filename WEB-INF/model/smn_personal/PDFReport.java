...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("psl_habilita_acceso");
			if (d != null)
			{
			name  = "b_enables_access_control";
			label = "${lbl:b_enables_access_control}";
		...
		d = rs.getString("psl_clase_auxiliar");
			if (d != null)
			{
		...
		d = rs.getString("psl_clase_auxiliar_name");
			if (d != null)
			{
			name  = "b_auxiliary_class";
			label = "${lbl:b_auxiliary_class}";
		...
		d = rs.getString("psl_num_control");
			if (d != null)
			{
			name  = "b_number_of_control";
			label = "${lbl:b_number_of_control}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
