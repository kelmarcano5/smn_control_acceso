package domain.repgen.smn_acceso.smn_movimiento_marcajes;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("mrk_auxiliar_name");
			if (d != null)
			{
			name  = "b_auxiliary";
		...
		d = rs.getString("mrk_marcaje_hora_fin");
			if (d != null)
			{
			name  = "b_transaction_time_less_than";
			label = "${lbl:b_transaction_time_less_than}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("mrk_marcaje_hora_ini");
			if (d != null)
			{
			name  = "b_transaction_time_grater_than";
			label = "${lbl:b_transaction_time_grater_than}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("mrk_marcaje_tipo");
			if (d != null)
			{
			name  = "b_device_type";
			label = "${lbl:b_device_type}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("mrk_marcaje_fecha_fin");
			if (d != null)
			{
			name  = "b_transaction_date_less_than";
			label = "${lbl:b_transaction_date_less_than}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("mrk_control_acceso_id");
			if (d != null)
			{
		...
		d = rs.getString("mrk_marcaje_fecha_ini");
			if (d != null)
			{
			name  = "b_transaction_date_grater_than";
			label = "${lbl:b_transaction_date_grater_than}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("mrk_marcaje_dia");
			if (d != null)
			{
			name  = "b_day_name";
			label = "${lbl:b_day_name}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
