package domain.repgen.smn_acceso.smn_resumen_marcajes;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("igs_fecha_ini");
			if (d != null)
			{
			name  = "s_loginhist_col1_grater_than";
			label = "${lbl:s_loginhist_col1_grater_than}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("igs_actividad");
			if (d != null)
			{
			name  = "b_activity_type";
			label = "${lbl:b_activity_type}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("igs_auxiliar_name");
			if (d != null)
			{
			name  = "b_auxiliary";
			label = "${lbl:b_auxiliary}";
		...
		d = rs.getString("igs_clase_auxiliar_name");
			if (d != null)
			{
			name  = "b_auxiliary_class";
			label = "${lbl:b_auxiliary_class}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("igs_clase_auxiliar");
			if (d != null)
			{
			name  = "b_auxiliary_class";
			label = "${lbl:b_auxiliary_class}";
		...
		d = rs.getString("igs_fecha_fin");
			if (d != null)
			{
			name  = "s_loginhist_col1_less_than";
			label = "${lbl:s_loginhist_col1_less_than}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("igs_actividad_name");
			if (d != null)
			{
			name  = "b_activity_type";
			label = "${lbl:b_activity_type}";
