package domain.repgen.smn_registro.smn_justificaciones;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("jus_fecha_incidencia_fin");
			if (d != null)
			{
			name  = "b_incidence_date_until";
			label = "${lbl:b_incidence_date_until}";
		...
		d = rs.getString("jus_fecha_incidencia_ini");
			if (d != null)
			{
			name  = "b_incidence_date_from";
			label = "${lbl:b_incidence_date_from}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("jus_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("smn_personal_id");
			if (d != null)
			{
			name  = "b_personal";
			label = "${lbl:b_personal}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_personal_id_name");
			if (d != null)
			{
			name  = "b_personal";
			label = "${lbl:b_personal}";
		...
		d = rs.getString("jus_aprobado_por");
			if (d != null)
			{
			name  = "b_approved_by";
			label = "${lbl:b_approved_by}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
