...
public class resumen {
	...
	public void leerListas(ArrayList<asistencia> asistencias, ArrayList<movimientoMarcaje> movimientosMarcajes, ArrayList<String[]> ids,ArrayList<String[]> listaIndividual_All,ArrayList<horarios> dias, String fechaDia,String nombreDia,ArrayList<String[]> tipoActividades){
...
//						if(leidas[4].equalsIgnoreCase("00:00") || leidas[4].equalsIgnoreCase("NA")) {
						...
						if(leidas[4]==null || leidas[4]==null) {
							...
							ArrayList<asistencia> AsistenciasSal_aux = this.cuantosTiposAsistenciaPorPersona(listaPorPersonaAs, "SAL");
							int plaAux = 0;
							boolean justEn=false;
							boolean justSal=false;
							if(AsistenciasEnt_aux.size()>0) {
								plaAux=AsistenciasEnt_aux.get(0).getClavePlanAsistencia();
								if(AsistenciasEnt_aux.get(0).getEstatusPermiso().equalsIgnoreCase("S")) {
									justEn=true;
								}
								/*
								if(AsistenciasEnt_aux.get(0).getClavePersonal()==561) {
									System.out.println(AsistenciasEnt_aux.get(0).getEstatusPermiso());
								}
								*/
							}

							/*							if(AsistenciasSal_aux.size()>0) {
								plaAux=AsistenciasSal_aux.get(0).getClavePlanAsistencia();
								if(AsistenciasSal_aux.get(0).getEstatusPermiso().equalsIgnoreCase("S")) {
									justSal=true;
								}
								if(AsistenciasSal_aux.get(0).getClavePersonal()==561) {
									System.out.println(AsistenciasSal_aux.get(0).getEstatusPermiso());
								}
							}
							if(AsistenciasEnt_aux.get(0).getClavePersonal()==561) {
								System.out.println("--"+justEn+"--");
								System.out.println("--"+justSal+"--");
							}*/
							
							if(justEn) {
								js= true;
							}else {
								int tipoAc = this.buscarEnTipoActividad("Retraso", tipoActividades);
								js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, horahorarioDia.getHor_hora_inicio(), rmk_tiempo_ausencias, persona_id[8], "Retraso Total", "CAUSA",plaAux);
							}
								...
								if(AsistenciasCom.size()==0) {//NO TIENE ASIGNADO NI MARCO EN COMEDOR
									...
									for (int jklm = 0; jklm < AsistenciasCom.size(); jklm++) {
										...
										if(difeEn>0) {//TARDE
											...
											if(coAsis.getEstatusPermiso().equalsIgnoreCase("S")) {
									
											}else {
												int tipoAc = this.buscarEnTipoActividad("Retraso", tipoActividades);
												boolean js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, coMrk.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada Comedor", "CAUSA",coAsis.getClavePlanAsistencia());
												if(!js) {
													noJustifico+=" "+persona_id[0]+" | "+fechaDia;
													falsesJ++;
												}else {
...
//											System.out.println("ENTRADA F : "+hm_difE);
											...
											if(coAsis.getEstatusPermiso().equalsIgnoreCase("S")) {
												...
												int tipoAc = this.buscarEnTipoActividad("Retraso", tipoActividades);
												boolean js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, comMrk.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada Comedor", "CAUSA",coAsis.getClavePlanAsistencia());
												if(!js) {
													noJustifico+=" "+persona_id[0]+" | "+fechaDia;
													falsesJ++;
												}else {
								...
								if(ultimoAsisCom>0) {
									for (int jklm = 0; jklm < AsistenciasCom.size(); jklm++) {
										...
										if(difeEn>0) {//TARDE
											...
											if(comAsis.getEstatusPermiso().equalsIgnoreCase("S")) {
											}else {
												int tipoAc = this.buscarEnTipoActividad("Retraso", tipoActividades);
												boolean js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, comMrk.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada Comedor", "CAUSA",comAsis.getClavePlanAsistencia());
												if(!js) {
...
//										System.out.println("ENTRADA F : "+hm_difE);
										...
										if(entAsis.getEstatusPermiso().equalsIgnoreCase("S")) {
										}else {
											int tipoAc = this.buscarEnTipoActividad("Retraso", tipoActividades);
											boolean js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, entMrk.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada", "CAUSA",entAsis.getClavePlanAsistencia());
											if(!js) {
												noJustifico+=" "+persona_id[0]+" | "+fechaDia;
												falsesJ++;
											}else {
...
//											System.out.println("ENTRADA F : "+hm_difE);
								...
								for (int ultAsis = ultimoMrk; ultAsis < AsistenciasEnt.size(); ultAsis++) {
									...
									if(falto_entradaPrincipal_asi) {
										...
										if(auxx.getEstatusPermiso().equalsIgnoreCase("S")) {
										}else {
											int tipoAc = this.buscarEnTipoActividad("Retraso", tipoActividades);
											js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, auxx.getHoraTopeEvento(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada", "CAUSA",auxx.getClavePlanAsistencia());
...
//											System.out.println("ENTRADA F : "+hm_difE);
								...
								for (int ultimoMrk = ultimoAsis; ultimoMrk < MarcajesEnt.size(); ultimoMrk++) {
									...
									int difeEn = rmk_tiempo_asistencia;
									int tipoAc = this.buscarEnTipoActividad("Asistencia no planificada", tipoActividades);
									boolean js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, auxx.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Asistencia no planificada", "CAUSA",0);
									if(!js) {
...
//									System.out.println(entAsis.getHoraTopeEvento()+" "+entMrk.getMarcajeHora());
									...
									if(difeSal>0) {//SALIO MAS TARDE 
										...
										int tipoAc = this.buscarEnTipoActividad("Tiempo adicional", tipoActividades);
										boolean js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, salMrk.getMarcajeHora(), this.calcularDif_DP(difeSal), persona_id[8], "Tiempo adicional", "CAUSA",salAsis.getClavePlanAsistencia());
										if(!js) {
											noJustifico+=" "+persona_id[0]+" | "+fechaDia;
											falsesJ++;
										}else {
										...
										String hm_difS = this.horasMin_restantes(difeSal);
										int abs = difeSal*(-1);
										int tipoAc = this.buscarEnTipoActividad("Retraso en salida", tipoActividades);
										boolean js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, salMrk.getMarcajeHora(), this.calcularDif_DP(abs), persona_id[8], "Retraso Salida", "CAUSA",salAsis.getClavePlanAsistencia());
										if(!js) {
											noJustifico+=" "+persona_id[0]+" | "+fechaDia;
											falsesJ++;
										}else {
...
//											System.out.println("ENTRADA F : "+hm_difE);
								...
								for (int ultAsis = ultimoMrk; ultAsis < AsistenciasSal.size(); ultAsis++) {
									...
									if(falto_entradaPrincipal_asi) {
										...
										int tipoAc = this.buscarEnTipoActividad("Retraso en salida", tipoActividades);
										js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, auxx.getHoraTopeEvento(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Salida", "CAUSA",auxx.getClavePlanAsistencia());
									}									
...
//											System.out.println("ENTRADA F : "+hm_difE);
											...
											int tipoAc = this.buscarEnTipoActividad("Tiempo adicional", tipoActividades);
											boolean js = this.insertarJustificacion(persona_id[0], tipoAc, "20", fechaDia, salMrk.getMarcajeHora(), this.calcularDif_DP(difeSa), persona_id[8], "Tiempo adicional", "CAUSA",salAsis.getClavePlanAsistencia());
											if(!js) {
...
//		System.out.println(s);
	...
	public int buscarEnTipoActividad(String nombreTipo,ArrayList<String[]> tipoActividades) {
		int idTipo = 0;
/*
 * 				data = {
            			("smn_tipo_actividades_id"),
            			("tya_codigo"),
            			("tya_nombre")
            	}
 * 
 */
		...
		if(tipoActividades.size()>0) {
    		for (int i = 0; i < tipoActividades.size(); i++) {
    			String[] obj = tipoActividades.get(i);
    			if(obj[2].equalsIgnoreCase(nombreTipo)) {
    				try {
        				int aux = Integer.parseInt(obj[0]);
        				idTipo = aux;
        				break;
    				}catch (Exception e) {
    					idTipo= 0;
...
//    					System.out.println(idPersonal+" "+mrk.getPersonal());
	...
	public boolean insertarJustificacion(String idPersonal,int tipoActividad,String docRf,String fechaIncidencia,String horaIncidencia,double timeJust,String jefe, String descripcion, String causa,int planA) {
		boolean rst = false;
        String horaActual = new SimpleDateFormat("HH:mm:ss").format(new java.util.Date());
        String fechaActual = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date());     			
