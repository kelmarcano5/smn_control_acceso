...
//						if(leidas[4].equalsIgnoreCase("00:00") || leidas[4].equalsIgnoreCase("NA")) {
						...
						if(leidas[4]==null || leidas[4]==null) {
							...
							js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, horahorarioDia.getHor_hora_inicio(), rmk_tiempo_ausencias, persona_id[8], "Retraso Total", "CAUSA");
							...
							if(AsistenciasCom.size()==MarcajesCom.size()) {//SI MARCO LA MISMA CNT DE EL PLAN 
								if(AsistenciasCom.size()==0) {//NO TIENE ASIGNADO NI MARCO EN COMEDOR
									...
									for (int jklm = 0; jklm < AsistenciasCom.size(); jklm++) {
										...
										if(difeEn>0) {//TARDE
											boolean js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, coMrk.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada Comedor", "CAUSA");
											boolean flsb = this.updateAsistencia(coAsis.getClaveAsistencia(), coMrk.getMarcajeHora(), "A");
											if(!flsb) {
												falses++;
...
//											System.out.println("ENTRADA F : "+hm_difE);
											...
											boolean js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, comMrk.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada Comedor", "CAUSA");
...
//										System.out.println("ENTRADA F : "+hm_difE);
										...
										boolean js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, entMrk.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada", "CAUSA");
...
//											System.out.println("ENTRADA F : "+hm_difE);
								...
								for (int ultAsis = ultimoMrk; ultAsis < AsistenciasEnt.size(); ultAsis++) {
									...
									if(falto_entradaPrincipal_asi) {
										...
										js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, auxx.getHoraTopeEvento(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Entrada", "CAUSA");
...
//											System.out.println("ENTRADA F : "+hm_difE);
								...
								for (int ultimoMrk = ultimoAsis; ultimoMrk < MarcajesEnt.size(); ultimoMrk++) {
									...
									int difeEn = rmk_tiempo_asistencia;
									boolean js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, auxx.getMarcajeHora(), this.calcularDif_DP(difeEn), persona_id[8], "Asistencia no planificada", "CAUSA");
									if(!js) {
...
//									System.out.println(entAsis.getHoraTopeEvento()+" "+entMrk.getMarcajeHora());
									...
									if(difeSal>0) {//SALIO MAS TARDE 
										...
										boolean js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, salMrk.getMarcajeHora(), this.calcularDif_DP(difeSal), persona_id[8], "Tiempo adicional", "CAUSA");
										if(!js) {
											noJustifico+=" "+persona_id[0]+" | "+fechaDia;
											falsesJ++;
										}else {
										...
										String hm_difS = this.horasMin_restantes(difeSal);
										int abs = difeSal*(-1);
										boolean js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, salMrk.getMarcajeHora(), this.calcularDif_DP(abs), persona_id[8], "Retraso Salida", "CAUSA");
										if(!js) {
											noJustifico+=" "+persona_id[0]+" | "+fechaDia;
											falsesJ++;
										}else {
...
//											System.out.println("ENTRADA F : "+hm_difE);
								...
								for (int ultAsis = ultimoMrk; ultAsis < AsistenciasSal.size(); ultAsis++) {
									...
									if(falto_entradaPrincipal_asi) {
										...
										js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, auxx.getHoraTopeEvento(), this.calcularDif_DP(difeEn), persona_id[8], "Retraso Salida", "CAUSA");
...
//											System.out.println("ENTRADA F : "+hm_difE);
											...
											boolean js = this.insertarJustificacion(persona_id[0], "0", "20", fechaDia, salMrk.getMarcajeHora(), this.calcularDif_DP(difeSa), persona_id[8], "Tiempo adicional", "CAUSA");
											if(!js) {
...
//    					System.out.println(idPersonal+" "+mrk.getPersonal());
	...
	public boolean insertarJustificacion(String idPersonal,String tipoActividad,String docRf,String fechaIncidencia,String horaIncidencia,double timeJust,String jefe, String descripcion, String causa) {
		boolean rst = false;
        String horaActual = new SimpleDateFormat("HH:mm:ss").format(new java.util.Date());
        String fechaActual = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date());     			
