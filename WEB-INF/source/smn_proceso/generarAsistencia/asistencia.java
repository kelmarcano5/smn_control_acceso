...
//      			System.out.println(sentenciaPersonalS);
      			...
      			while(rs.next()){
			        ...
			        try {
			            ...
			            while (cfin.after(cini) || cfin.equals(cini)) {
		        			...
		        			horarios horasAus = new horarios(this.getFw());
			            ...
			            if((cantidadDias-contadorDias)!=0) {
			            	this.fw.write(timestamp+":No se inserto completamente las asistencias en smn_plan_asistencia ID:("+idPlanAsistencia+") al personal con el ID ("+rs.getString("smn_personal_id")+")");
			            	this.fw.write(timestamp+":Cantidad DIAS del plan: "+cantidadDias);
			            	this.fw.write(timestamp+":Cantidad DIAS del plan NO insertados: "+(cantidadDias-contadorDias));
			            ...
			            e2.printStackTrace();
			            this.fw.write(timestamp+":Error al recorrer las fechas de smn_plan_asistencia ID:("+idPlanAsistencia+")");
			            this.fw.write(timestamp+":"+e2);
	                	
			        }
      			}//DEL WHILE
      			this.fw.write(timestamp+":Cantidad de personal en este smn_plan_asistencia ID:("+idPlanAsistencia+"): "+(cantidadPersonal));	                	
      			this.fw.write(timestamp+":Cantidad de asistencias Generadas: "+(contadorDias)+" *4 = "+(contadorDias*4));	                	
			...
			this.fw.write(timestamp+":Error al generar las asistenca del smn_control_acceso.smn_plan_asistencia.smn_plan_asistencia_id="+idPlanAsistencia);
			this.fw.write(timestamp+":ERROR: "+e);
		}
	...
	public boolean cambiarEstatusPlan(String idPlanAsistencia) throws Throwable {
		...
		try {
			...
			this.fw.write(timestamp+":Error al cambiar el 'cpa_status' a 'GE' en el plan: smn_control_acceso.smn_plan_asistencia="+idPlanAsistencia);
			this.fw.write(timestamp+":ERROR: "+e);
		}
